...
def cleaning_processing(date_min, date_max, config):
    ...
    for i, df_hen in tqdm.tqdm(df.groupby(['HenID'])):
        ...
        for r in range(1,6):
            df_hen['next'+str(r)+'_record_date'] = df_hen['Timestamp'].tolist()[r:]+[np.nan for v in range(1,r+1)]
        ...
        for r in range(1,6):
            df_hen['previous'+str(r)+'_record_date'] = [np.nan for v in range(1,r+1)]+df_hen['Timestamp'].tolist()[0:-r]
...
def ShapiroTest(data, alpha=0.05):
    '''The tests assume that that the sample was drawn from a Gaussian distribution, and refect hypothesis if not drwan from a normal distribution'''
    stat, p = shapiro(data)
...
def preprocessing_Origins(paths, config, save=True, dodevice=True):
    ...
    path_FocalBirdInfo: should be the path ending with .xlsx'''
    ...
    if dodevice==True:
        ...
        df_lastactionGAP = DefineWeridTagDays(config, df_device)
    ...
    df = df.filter(['Timestamp', 'HenID', 'Zone','PenID','log_file_name','ts_order','TagID','signalstrength','system',
                'time','date']).reset_index(drop=True)
...
def DefineWeridTagDays(config, df_device):
...
def device_Variables(config, df_device):
    ...
    df_device_day['level'] = df_device_day['New_Timestamp'].map(lambda x: dt.datetime.date(x))
    #display(df_device_day.head(3))
...
def preprocessing_experiment2(paths, path_FocalBird, config, save=True):
    ...
    ############### Download info on henID associtation to (TagID,date) ################
    ####################################################################################
    df_FB = pd.read_csv(path_FocalBird, sep=';', parse_dates=['StartDate','EndDate'], dayfirst=True) #date verified correct
    #fill end date to today+1 for the birds which we dont know when is there end date (+1: so that today is taken into account)
...
def general_cleaning_Origins(df, config, save=True):
    ...
    *Output: the exact same csv, with few new columns, Timestamp columns has the microseconds and Initial_timestamp is the initial one (i.e. without microseconds)'''
    ...
    print_color((('-----------------------------------------------------------------------------------------------','blue'),))
    print_color((('remove consecutives equal zone for the same hen.........','blue'),))
    print_color((('-----------------------------------------------------------------------------------------------','blue'),))
...
#e.g. of usage
#target will be the output of kmeans clustering
...
def dict_of_zones_appearances_with_transitionalZones(li):
    #remove teh effecct of duration
...
def dico_duration_stats(li, nbr_sec):
    ...
    dico_dur_stats =  {'mean_duration':np.mean(v), 'median_duration':np.median(v), 'max_duration':max(v),
                       ...
                       'percentile_15':np.percentile(v,15,interpolation='lower'),
                       'percentile_85':np.percentile(v,85,interpolation='lower'),
                       'percentile_95':np.percentile(v,95,interpolation='lower'),
...
def is_WG_open(x, dico_, date_first_opening_WG, epsi_open=0, epsi_close=20):
    '''from a timestamp value x, the dico_garden_opening_hour and the date_first_opening_WG parameters, it will output true if 
    the WG is open, false otherwise.
    With the epsi_* paramteres it allows to be more flexible with the true time of opening/closing'''
    #if no record return nan
    if pd.isnull(x)==True:
        return(np.nan)
    if x<date_first_opening_WG:
        return(False)
    if max(dico_.keys())<dt.datetime(x.year,x.month,x.day,0,0,0):
...
def is_ts_before_MN_opening(x, dico_, MN, date_first_opening_WG, epsi_open=0):
...
def HenDailyVariable_Origins(df, config, name_='', timestamp_name='Timestamp', save=True, compute_chi2_distance=False,
                             time4entropy=False, has_cons_equal_zone=True): 
    
    ''' 
    #ATTENTION TO BE DONE BEFORE: consecutives records of same zone should be removed, otherwise the night variable will be wrong!!
    ...
    if not has_cons_equal_zone:
        ...
        #first, create the nbr_transition_next1hafterlightoffmax_duration_zones
    ...
    df_daily['duration_last-firsttransition_mn'] = df_daily['duration_last-firsttransition_mn'].map(lambda x: round(x.tm_hour*60+x.tm_min+x.tm_sec/60,0) if type(x)==time.struct_time else np.nan)
    
    ########################################################
    #### WG
    ########################################################    
    #went out after its 15mn opening?
    df_daily['in_WG_'+str(WG_after_opening_mn)+'mnAfterOpening'] = df_daily['FirstTimestamp_1_Zone'].map(lambda x: is_ts_before_MN_opening(x, dico_garden_opening_hour, WG_after_opening_mn, date_first_opening_WG))
    ...
    #ATTENTION; ASSUMPTION:Each henID is linked to a unique PenID!
    li_weight = [i for i in df_FB.columns if 'weight' in i]
    li_var_hen = ['HenID','PenID','CLASS','TagID','FocalLegringName','R-Pen','InitialStartDate']+li_weight
    #some hens appeared several times (e.g. changed tags, changed legrings,...), so first we will join all info
    df_FB_ = df_FB.groupby(['HenID'])[[i for i in li_var_hen if i!='HenID']].agg(lambda x: list(x)[0] if len(set(x))<=1 else\
                                                                                 str(list(x))).reset_index()
    ...
    ######## remove dates linked to specific hens on the day
    print('-------------- Lets remove dates that impacted few hens')
    #create a dictionary with henID as keys and a list of tracking-active days, it takes into account if there is several rows for one hen
    dico_hen_activedate = defaultdict(list)
    for i in range(df_FB.shape[0]):
        x = df_FB.iloc[i]
        li_dates = pd.date_range(start=x['StartDate']+dt.timedelta(days=1), 
    ...
    df_daily['level'] = df_daily['level'].map(lambda x: dt.datetime(x.year,x.month,x.day)) 
    ...
    df_wt['sender'] = df_wt['sender'].map(lambda x: 'tag_'+str(int(x)))
    x0 = df_daily.shape[0]
    #fillnan with big number to it also means weird values if there is only nan
    df_wt['biggest_gap_of_day_mn'].fillna(999,inplace=True)
    df_daily['date_2remove_weirdupdate'] = df_daily.apply(lambda x: x['level'] in df_wt[(df_wt['biggest_gap_of_day_mn']>12)&\
    ...
    if save:
        print('save')
        df_daily.drop(['verification_daily_total_nbr_hour','zone_list','date_2remove_penper','date_2remove_penhen',
