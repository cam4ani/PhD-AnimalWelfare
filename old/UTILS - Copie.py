...
def preprocessing_origins(paths, FocalBirdInfo, config, save=True):
    ...
    li_df = []
    for path in paths:
        df = pd.read_csv(path_log, sep=';',names=['date','TagerialNumber','TagID','zone','systemid','useless_zone','signalstrength',
                                                  'zone2','signalstzone2','zone3','signalstrzone3','zone4','signalstrzone4']) 
        df.columns = col_names
        df['data_path'] = path
        df['system'] = path.split('Barn 4 Pen ')[1].split('\\')[0]
    ...
    df['month'] = pd.DatetimeIndex(df['Timestamp']).month.astype(int)
    df['day'] = pd.DatetimeIndex(df['Timestamp']).day.astype(int)
    df['TagID'] = df['TagID'].astype(str)
    print(df.shape)
    
    #active tags --> pen~tag info
    df_tag = pd.read_excel(p_pen_info) #,parse_dates=['startdate'] faux!!
    df_tag['startdate'] = df_tag['startdate'].map(lambda x: dt.datetime.strptime(x, "%d.%m.%Y"))
    #fill end date to today for the birds which we dont know when is there end date
    df_tag['enddate'] = df_tag['enddate'].map(lambda x: dt.datetime.strptime(x, "%d.%m.%Y") if math.isnan(x)==False else np.nan)
    df_tag['enddate'].fillna(dt.date.today(), inplace=True)
    df_tag['TagID'] = df_tag['TagID'].astype(str)
    li_active_tags = list(df_tag[df_tag['enddate']>=dt.date.today()]['TagID'].unique())
    li_serialnbr_activetags = list(df_tag[df_tag['enddate']>=dt.date.today()]['TagSerialNumber'].unique())
    #Counter(li_active_tags)
    print('You have %d ative tags'%len(li_active_tags))

    #make sure about the type
    df['Zone'] = df['Zone'].map(lambda x: x.strip())
    df = df.sort_values(['Timestamp'], ascending=True)
    #TOCHANGE:
    df['HenID'] = df['TagID'].copy()
    df['PenID'] = df['HenID'].map(lambda x: df_tag[df_tag['TagID']==x]['PenID']) #first digits matching the string
...
def verification_based_on_initial_record(df, config, min_daily_record_per_hen=500, last_hour_outside2inside=17, min_nbr_zone_per_day=5,
    ...
    ###########################################################################################################################
    print_color((('-----------------------------------------------------------------------------------------------','blue'),))
    print_color((('change zone name sinto egenral zone names, add pen info and look at basic info.........','blue'),))
    print_color((('-----------------------------------------------------------------------------------------------','blue'),))
