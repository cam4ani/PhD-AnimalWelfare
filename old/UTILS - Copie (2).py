...
def preprocessing_origins(paths, path_FocalBirdInfo, config, save=True):
    ...
    ####################################################################################
    df_FB = pd.read_excel(path_FocalBirdInfo, parse_dates=['StartDate','EndDate'])
    #fill end date to today+1 for the birds which we dont know when is there end date (+1: so that today is taken into account)
    df_FB['EndDate'].fillna(dt.date.today()+dt.timedelta(days=1), inplace=True)
    ...
    li_active_tags = list(df_FB[df_FB['EndDate']>=dt.date.today()]['TagID'].unique())
...
def preprocessing_experiment2(paths, path_FocalBirdInfo, config, save=True):
    ...
    df_pen_info = pd.read_excel(path_FocalBirdInfo,sep=';',index_col=False) 
    ...
    df['PenID'] = df['HenID'].map(lambda x: re.search(r'\d+', x).group()) #first digits matching the string
...
def time_series_henColumn_tsRow(df, config, col_ts='Zone' , name_='', ts_with_all_hen_value=True, save=True, 
    ...
    #print('-------------- Lets remove unwanted dates')
    ...
    #df_hens['hour'] = df_hens['Timestamp'].map(lambda x: x.hour if pd.isnull(x)!=np.nan else np.nan)
        ...
        #df_hens['date_toberemoved'] = df_hens['date'].map(lambda x: x in li_date2remove)
...
def BADOneTimeSeriesPlot(df, config, timestamp_name='New_Timestamp',  value='most_frequent_zone', save=True, 
                      last_folder_name='', name_='',l=25, c=3):
    
    '''For one hen timeseries plot the time series over the whole period
    df_ts should have at least those columns: timestamp_name, value, date, hen_i, hen_j,...'''
    ...
    #convert to float as sometimes its float (when less or more zones)
...
def TimeSeriesPlot_1row1day(df, config, save=True, timestamp_name='New_Timestamp', last_folder_name='', name_=''):
    ...
    for hens in tqdm.tqdm(li_hen):
        ...
        for month,df_plt_ in df_plt.groupby('month'):
            ...
            if save:
                plt.savefig(os.path.join(path_,id_run+'_TimeSeries_'+name_+'_month'+str(month)+'_'+hens+'.png'), dpi=300, format='png',
                            bbox_inches='tight')       
            #plt.show();
            plt.close()
...
def HenVariable(df, config, path_FocalBird, ts_name, name_='', nbr_sec_chaoticmvt_notmiddle=5*60, nbr_sec_chaoticmvt_middle=2*60,
    ...
    We assume that the df_ts produced on the first row has equally spaced timestamp values and that nbr_sec is the duration in between each timestamp (by construction should be the case)
    ...
    df_ = df[~df['Zone'].isnull()].groupby(['HenID','level']).agg(
           ...
           nbr_bouts=pd.NamedAgg(column='Zone', aggfunc=lambda x: nbr_bouts_per_zone(list((x)))),
    ...
    #the HenID was already match according to the correct dates
    ...
    #remove dates linked to specifc system
...
def HenEntropy(df_ts, config, ts_name, name_='', nbr_sec_chaoticmvt_notmiddle=5*60, nbr_sec_chaoticmvt_middle=2*60,
    ...
    START_TIME = time.clock() #start recording the time it last
    path_extracted_data = config.path_extracted_data
    id_run = config.id_run
    dico_night_hour = config.dico_night_hour
    dico_zone_order = config.dico_zone_order
